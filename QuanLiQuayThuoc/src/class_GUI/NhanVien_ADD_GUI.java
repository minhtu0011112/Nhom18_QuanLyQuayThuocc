...
public class NhanVien_ADD_GUI extends JFrame {
	 ...
	 */
	public static void NewScreen(int thongtin) {
		thongtin1 = thongtin;
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NhanVien_ADD_GUI frame = new NhanVien_ADD_GUI();
...
//		horizontalBox.add(label_2);
		...
		btnLuu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(kiemTraDinhDang()) {
					...
					DangNhap dn;
					if(nv.getChuVu().trim().equalsIgnoreCase("Nhân Viên"))
						dn = new DangNhap(maNV, "000000", 0);
					else 
						dn = new DangNhap(maNV, "000000", 1);					
					try {
						if(!qlnv_DAO.addNV(nv))
							throw new Exception();
						else {
							if(!dn_DAO.addTK(dn)) {
								throw new Exception();
	...
	private boolean kiemTraDinhDang() {
		...
		if (ma.matches("\\s*")) {
			...
			JOptionPane.showMessageDialog(this, "Chức vụ của nhân viên?");
			return false;
		}
		for(NhanVien s : NhanVien_GUI.lstnvall) {
			if(s.getMaNV().trim().equals(ma)) {
				JOptionPane.showMessageDialog(null, "Mã nhân viên đã tồn tại trước đó!!");
				return false;
			}
		}
		return true;
