...
public class HoaDon_ADD1_GUI extends JFrame {
...
//			public void run() {
//				try {
//					HoaDon_ADD_GUI frame = new HoaDon_ADD_GUI();
...
//	}
	 ...
	 * Create the frame.
	 */
	public HoaDon_ADD1_GUI(String maDN) {
				...
				if(validData()) {
					...
					try {
						...
						for(int i=0; i<model.getRowCount(); i++) {
							cthd = new CTHoaDon(maHD, model.getValueAt(i, 1).toString(), Integer.valueOf(model.getValueAt(i, 6).toString()), hinhThucBan, Float.valueOf(model.getValueAt(i, 7).toString()), chietKhau, khachDua, traKhach, 2);
							if(!cthd_DAO.create(cthd))
								throw new Exception();	
						}
						//DocDLDB();
