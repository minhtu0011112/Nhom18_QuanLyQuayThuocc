...
public class KhachHang_ADD_GUI extends JFrame {
	...
	public static void NewScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					KhachHang_ADD_GUI frame = new KhachHang_ADD_GUI();
...
//		horizontalBox.add(label_2);
		...
		lblNewLabel.setToolTipText("M\u00E3 KH:");
		lblNewLabel.setHorizontalAlignment(SwingConstants.RIGHT);
		lblNewLabel.setBounds(10, 34, 91, 27);
		...
		txtLoaiKH.setBounds(395, 37, 173, 27);
		...
		btnLuu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(validData()) {
					...
					//KhachHang kh = new KhachHang(txtMaKH.getText().trim(), txtLoaiKH.getText().trim(),txtHoTen.getText().trim(),datePicker1.getJFormattedTextField().getText(), null, txtDiaChi.getText().trim(),txtTinhTP.getText().trim(),txtQH.getText().trim(),txtSDT.getText().trim(),txaGhiChu.getText().trim(), ghiChu );
					try {
						if(!kh_DAO.create(kh))
							throw new Exception();
